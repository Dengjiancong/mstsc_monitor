...
本项目只支持 Windows。想直接使用时，到 [Releases](https://github.com/Dengjiancong/mstsc_monitor/releases/latest) 下载 `MSTSC_Monitor.exe`，单独放进一个文件夹后双击运行，不需要安装 Python。首次启动请填写飞书应用信息，并在 A～D 标签页配置需要的 mstsc 任务；个人配置、截图和可选 GIF 会保存在 exe 所在文件夹。
...
首次启动需要在程序内填写自己的飞书应用信息，在 A～D 标签页分别启用需要的任务、选择对应 mstsc 窗口并框选状态框。`completion_config.json` 由程序在运行时生成，**不在仓库中**；截图、待补发记录以及个人 GIF 也不在仓库中。旧版单任务配置首次打开时会迁移到 A 任务，B～D 默认关闭。

每个启用任务独立监控所选 mstsc 窗口的状态框。触发规则分三种：**启动时若已是紫色 credence**，先等待检测到一次绿色 Testing，再连续检测三次紫色后截图提醒；**启动时若是绿色 Testing**，首次检测到紫色就截图提醒；**提醒过之后**，同一段紫色不会重复发送，下一轮仍需要一次绿色、连续三次紫色才再次提醒。触发时按顺序发送完整 mstsc 截图、带任务名的**截图时间**（例如 `A设备任务｜2026年9月16日 08:24:52`）、带任务名的温柔结束文字；开启“发送随机 GIF 表情包”时，最后从同目录 `gif` 文件夹随机选一个 GIF 发送。时间取触发截图时，而不是飞书补发时；提醒只说明**测试结束**，不判断结果通过或失败。各任务的监控线程独立运行，飞书发送在统一后台队列中依次完成。
...
2. 打开需要监控的 mstsc；在 A～D 标签页启用需要的任务，点击“刷新窗口”，给每个任务选择不同的目标窗口。不使用的任务取消“启用此任务”。程序会保存每个任务的窗口标题，并在下次启动时尽量自动匹配；窗口标题相同或发生变化时请手动核对。
3. 让对应 mstsc 画面可见，点击“框选状态框”，框住**右上角**绿色 Testing / 紫色 credence 区域。原则上每个窗口分别框选；若多个窗口尺寸、远程分辨率和内部布局完全相同，可点击“复用其他选区”，再用“检查截图”验证。每个任务最终独立保存自己的选区。
4. 点击“检查截图”。程序会打开刚保存的截图。此时监控程序本身已遮挡 mstsc；确认截图仍显示**真实的远程画面**，而不是黑图、旧画面或监控程序窗口。截图保存在 `screenshots` 文件夹。
5. 切换到目标标签页，点击“测试当前任务飞书发送”，确认收到带任务名的测试消息。随后分别点击各任务的“开始监控”；每个标签页都有独立状态灯：正常取图为亮绿色，异常为黄色，未监控、关闭或手动停止为亮红色。电脑保持开机，mstsc 保持打开且不要最小化。
...
- “清理全部截图”是四个任务共用的按钮。先停止所有任务，等待截图、发送及待补发任务完成；确认后才会删除 `screenshots` 文件夹里程序生成的 `completion_*.png` 图片。其他文件和 `gif` 文件夹不受影响。删除后无法恢复，程序不会自动清理。
...
程序可以通过机器人私聊命令截取指定任务，不需要 NAS、公网 IP 或内网穿透。点击“飞书私聊截图指令”里的“开启指令”后，电脑会主动与飞书建立一条长连接；只有该程序运行且连接正常时，指令才能生效。指令与颜色监控分开运行，因此任务不必处于监控状态，但该任务必须已启用、已选择 mstsc 窗口，且窗口没有最小化。
...
英文部分不区分大小写，`jtA`、`JtB` 等混合写法也可识别。命令必须完整匹配；旧版无任务字母的 `截图`、`jt`、`JT` 不再触发。目标任务已关闭或尚未选择窗口时，机器人会回复具体原因。
...
3. 在程序“允许截图者 User ID / Open ID”里填**你自己的**飞书 User ID；若你当前用“私聊：User ID”接收提醒，升级后会默认填入该地址，仍请核对。也可填 `ou_` 开头的 Open ID。点击“开启指令”，待状态显示“飞书指令已连接”。
4. 程序收到你的指令后，会截取字母对应任务的 mstsc 窗口，保存本地 PNG，然后把带任务名的截图说明发回**你发指令的私聊**。同事用自己的飞书账号发送有效指令时，程序不会截图，会回复**“此指令未授权”**。群聊指令、其他文字和机器人自身消息都不会触发截图。
...
随机 GIF 表情包初始默认关闭。勾选或取消程序中的“发送随机 GIF 表情包”即可控制所有任务后续发送；选择会保存。飞书私聊只识别上表中带 A～D 的截图命令，输入“关闭表情包”等其他文字不会改变程序设置。远程截图指令不会发送随机 GIF，也不会被当作“测试已结束”提醒。
...
- 每个任务分别计算触发状态：提醒后必须再次看到一次绿色 Testing，下一次连续看到三次紫色 credence 才会再次提醒；启动时已经是紫色则先等待新一轮绿色，不会马上提醒。
